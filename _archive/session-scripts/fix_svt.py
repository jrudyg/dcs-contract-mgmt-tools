...
BAK = CSV.with_suffix('.csv.bak_svt')
...
with open(CSV, encoding='utf-8-sig', newline='') as f:
    ...
    for row in reader:
        ...
        # Remove the SVT (bare) folder row -- keep SVT Robotics
        if vf == 'SVT':
            removed += 1
            continue

        # Clear ManualReview on the SVT Robotics row
        if vf == 'SVT Robotics' and row.get('ManualReview','').strip() == 'MANUAL_REVIEW':
            row['ManualReview'] = ''
